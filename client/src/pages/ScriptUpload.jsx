...
const ScriptUpload = () => {
  ...
  const inputCls = "w-full p-3 border border-gray-200 rounded-xl text-base focus:ring-2 focus:ring-[#1e3a5f] focus:border-transparent transition";
    ...
    <div className="max-w-2xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-5 sm:mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1e3a5f]">Upload Your Script</h1>
            <p className="text-sm sm:text-base text-gray-500">Share your creative work with investors and producers</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-5">
          {[1, 2, 3].map(s => (
            <button key={s} onClick={() => setStep(s)}
              className={`flex-1 h-2 rounded-full transition ${step >= s ? "bg-[#1e3a5f]" : "bg-gray-200"}`} />
          ))}
        </div>
        <p className="text-sm text-gray-500 font-medium mb-4">Step {step} of 3 — {step === 1 ? "Basic Info" : step === 2 ? "Details & Roles" : "Pricing & Upload"}</p>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                  className="space-y-4">
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Script Title *</label>
                  ...
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Logline / Description *</label>
                  ...
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Synopsis</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-base text-gray-700 font-semibold mb-2">Genre *</label>
                    ...
                    <div>
                      <label className="block text-base text-gray-700 font-semibold mb-2">Content Type *</label>
                  ...
                  <div className="flex justify-end pt-2">
                    <button type="button" onClick={() => setStep(2)}
                      className="px-6 py-2.5 bg-[#1e3a5f] text-white rounded-xl text-sm font-medium hover:bg-[#162d4a] transition">
                      Next →
                    </button>
                  </div>
                </motion.div>
              ...
              {step === 2 && (
                <motion.div key="step2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                  className="space-y-4">
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Tags</label>
                  ...
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Cover Image URL</label>
                  ...
                  {/* Roles section */}
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2.5">Character Roles (for Talent Attachment)</label>
                    {roles.length > 0 && (
                      <div className="space-y-2 mb-3">
                        {roles.map((r, idx) => (
                          <div key={idx} className="flex items-center gap-2 bg-[#1e3a5f]/5 rounded-xl p-3">
                            <div className="flex-1 min-w-0">
                            <span className="text-base font-bold text-gray-800">{r.characterName}</span>
                            <span className="text-sm text-[#1e3a5f] font-semibold ml-2">{r.type}</span>
                            {r.gender && <span className="text-sm text-gray-500 ml-2">({r.gender})</span>}
                            </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 space-y-2">
                      ...
                      <button type="button" onClick={addRole} disabled={!newRole.characterName || !newRole.type}
                        className="w-full py-2.5 bg-[#1e3a5f]/10 text-[#1e3a5f] rounded-lg text-base font-semibold hover:bg-[#1e3a5f]/20 transition disabled:opacity-40">
                        + Add Role
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 justify-between pt-2">
                    <button type="button" onClick={() => setStep(1)}
                      className="px-7 py-3 border border-gray-200 text-gray-600 rounded-xl text-base font-medium hover:bg-gray-50 transition">← Back</button>
                    <button type="button" onClick={() => setStep(3)}
                      className="px-7 py-3 bg-[#1e3a5f] text-white rounded-xl text-base font-semibold hover:bg-[#162d4a] transition">Next →</button>
                  </div>
                </motion.div>
              )}
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-base text-gray-700 font-semibold mb-2">Price (USD) *</label>
                    ...
                    <div>
                      <label className="block text-base text-gray-700 font-semibold mb-2">Budget Level</label>
                  ...
                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Hold Fee (USD)</label>
                    <input type="number" name="holdFee" value={formData.holdFee} onChange={handleChange} min="0" placeholder="200" className={inputCls} />
                    <p className="text-sm text-gray-400 mt-1.5">Fee for producers to place a 30-day hold on your script (10% platform cut)</p>
                  </div>

                  <div>
                    <label className="block text-base text-gray-700 font-semibold mb-2">Script File URL *</label>
                    ...
                    <p className="text-sm text-gray-400 mt-1.5">Upload to cloud storage and paste the public link</p>
                  </div>

                  <div className="flex items-center gap-3 p-3 sm:p-4 bg-[#1e3a5f]/5 rounded-xl">
                    <input type="checkbox" name="isPremium" checked={formData.isPremium} onChange={handleChange} id="isPremium" className="w-5 h-5 text-[#1e3a5f] rounded" />
                    <label htmlFor="isPremium">
                      <span className="text-base font-bold text-gray-800">Mark as Premium Content</span>
                      <p className="text-sm text-gray-500">Featured placement and exclusive exposure</p>
                    </label>
                  </div>

                  {/* Summary */}
                  <div className="bg-gray-50 rounded-xl p-4">
                    <h4 className="text-base font-bold text-gray-700 mb-3">Upload Summary</h4>
                    <div className="grid grid-cols-2 gap-2.5 text-sm text-gray-600">
                      <span>Title:</span><span className="font-semibold text-gray-900">{formData.title || "—"}</span>
                      <span>Genre:</span><span className="font-semibold">{formData.genre || "—"}</span>
                      <span>Content Type:</span><span className="font-semibold capitalize">{formData.contentType?.replace("_", " ")}</span>
                      <span>Price:</span><span className="font-semibold">${formData.price || "0"}</span>
                      <span>Roles:</span><span className="font-semibold">{roles.length}</span>
                      <span>Hold Fee:</span><span className="font-semibold">${formData.holdFee || "200"}</span>
                    </div>
                  </div>

                  <div className="flex gap-3 pt-1">
                    <button type="button" onClick={() => setStep(2)}
                      className="px-6 py-3 border border-gray-200 text-gray-600 rounded-xl text-base font-medium hover:bg-gray-50 transition">← Back</button>
                    <button type="submit" disabled={loading}
                      className="flex-1 py-3 bg-[#1e3a5f] text-white rounded-xl text-base font-bold hover:bg-[#162d4a] transition disabled:opacity-50 shadow-sm">
                      {loading ? "Uploading..." : "Upload Script"}
                    </button>
                    <button type="button" onClick={() => navigate("/dashboard")}
                      className="px-6 py-3 border border-gray-200 text-gray-600 rounded-xl text-base font-medium hover:bg-gray-50 transition">
                      Cancel
                    </button>
                  </div>
                </motion.div>
