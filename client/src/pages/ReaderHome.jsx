...
const GENRE_OPTS = [
  ...
  { label: "Thriller", emoji: "🔪" }, { label: "Romance", emoji: "❤️" },
  { label: "Sci-Fi", emoji: "🚀" }, { label: "Fantasy", emoji: "🧙" },
  { label: "Mystery", emoji: "🔍" }, { label: "Adventure", emoji: "🗺️" },
  { label: "Crime", emoji: "🚔" }, { label: "Animation", emoji: "🎨" },
  { label: "Documentary", emoji: "🎥" }, { label: "Historical", emoji: "🏛️" },
  { label: "Biographical", emoji: "📖" }, { label: "Sports", emoji: "⚽" },
  { label: "Musical", emoji: "🎵" }, { label: "Family", emoji: "👨‍👩‍👧" },
  { label: "Psychological", emoji: "🧠" }, { label: "Dark Comedy", emoji: "🖤" },
];
...
const QuickPrefModal = ({ dark, initialGenres, initialTypes, onSave, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className={`flex items-center justify-between px-5 py-4 border-b shrink-0 ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${dark ? "bg-[#1e3a5f]/30" : "bg-[#1e3a5f]/10"}`}>
            ...
            <div>
              <p className={`text-[14px] font-bold leading-tight ${dark ? "text-white" : "text-gray-900"}`}>Set Your Preferences</p>
              <p className={`text-[11px] ${dark ? "text-white/30" : "text-gray-400"}`}>Your feed will show matching content first</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className={`w-8 h-8 flex items-center justify-center rounded-lg transition-colors ${dark ? "text-white/30 hover:text-white/60 hover:bg-white/[0.06]" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100"}`}
          >
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-[12px] font-bold uppercase tracking-widest ${dark ? "text-white/40" : "text-gray-500"}`}>Genres</p>
              {genres.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-md ${dark ? "bg-[#1e3a5f]/30 text-[#7aafff]" : "bg-[#1e3a5f]/10 text-[#1e3a5f]"}`}>{genres.length} selected</span>
                  <button onClick={() => setGenres([])} className={`text-[11px] font-semibold hover:underline ${dark ? "text-white/25 hover:text-white/50" : "text-gray-400 hover:text-gray-600"}`}>Clear</button>
                </div>
              )}
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              {GENRE_OPTS.map(({ label, emoji }) => (
                <button
                  key={label}
                  type="button"
                  onClick={() => toggle(genres, setGenres, label)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl text-[12px] font-semibold border-2 transition-all text-left ${
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-[12px] font-bold uppercase tracking-widest ${dark ? "text-white/40" : "text-gray-500"}`}>Content Types</p>
              {types.length > 0 && (
                <div className="flex items-center gap-2">
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-md ${dark ? "bg-[#1e3a5f]/30 text-[#7aafff]" : "bg-[#1e3a5f]/10 text-[#1e3a5f]"}`}>{types.length} selected</span>
                  <button onClick={() => setTypes([])} className={`text-[11px] font-semibold hover:underline ${dark ? "text-white/25 hover:text-white/50" : "text-gray-400 hover:text-gray-600"}`}>Clear</button>
                </div>
              )}
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              {TYPE_OPTS.map(({ label, display, emoji }) => (
                <button
                  key={label}
                  type="button"
          ...
          {(genres.length > 0 || types.length > 0) && (
            <div className={`flex items-start gap-2 p-3 rounded-xl border ${dark ? "bg-[#1e3a5f]/10 border-[#1e3a5f]/25" : "bg-[#1e3a5f]/[0.04] border-[#1e3a5f]/15"}`}>
              ...
              <p className={`text-[11px] leading-relaxed ${dark ? "text-[#7aafff]/70" : "text-[#1e3a5f]/70"}`}>
        ...
        {/* Footer */}
        <div className={`px-5 py-4 border-t shrink-0 flex items-center gap-2.5 ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
          <button
            onClick={onClose}
            className={`flex-1 px-4 py-2.5 rounded-xl border text-[13px] font-semibold transition-colors ${dark ? "bg-white/[0.04] text-white/50 border-white/[0.08] hover:bg-white/[0.08]" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 px-4 py-2.5 rounded-xl bg-[#1e3a5f] text-white text-[13px] font-bold hover:bg-[#162d4a] transition-colors disabled:opacity-60 flex items-center justify-center gap-2 shadow-md shadow-[#1e3a5f]/20"
          >
            {saving
...
const ReaderHome = () => {
  ...
  return (
    <div className="min-h-screen pb-16">
      <div className="max-w-6xl mx-auto px-4 mt-2 space-y-10">
        ...
        <div className={`rounded-2xl border p-4 ${dark ? "bg-[#0d1829] border-white/[0.06]" : "bg-white border-gray-200/70 shadow-sm"}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-[14px] font-bold ${dark ? "text-white/80" : "text-gray-800"}`}>
                {forYouActive && hasPrefs
                  ? `${matchCount} match${matchCount !== 1 ? "es" : ""} found for your preferences`
                  : "Personalised For You"}
              </p>
              <p className={`text-[12px] mt-0.5 truncate ${dark ? "text-white/30" : "text-gray-400"}`}>
                {hasPrefs
                  ? forYouActive
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-bold border transition-all duration-200 shrink-0 ${
                forYouActive
            ...
            <button
              ...
              className={`shrink-0 inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-[13px] font-bold border transition-all ${
                hasPrefs
            ...
            {forYouActive && !hasPrefs && (
              ...
              >
                <div className={`flex items-center gap-3 pt-3 mt-3 border-t ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
                  ...
                  <p className={`text-[13px] flex-1 ${dark ? "text-white/50" : "text-gray-500"}`}>
                    You haven't set any preferences yet.
                  </p>
                  <button
                    onClick={() => setShowPrefModal(true)}
                    className={`shrink-0 px-3.5 py-1.5 rounded-xl text-[12px] font-bold border transition-all ${
                      dark
            ...
            {forYouActive && hasPrefs && (
              ...
              >
                <div className={`flex items-center gap-2 flex-wrap pt-3 mt-3 border-t ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
                  <span className={`text-[11px] font-bold uppercase tracking-widest shrink-0 ${dark ? "text-white/25" : "text-gray-400"}`}>
                    Filtering by
                  </span>
                  {prefGenres.map((g) => (
                    <span key={g} className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold border capitalize ${
                      dark ? "bg-[#1e3a5f]/20 text-[#7aafff] border-[#1e3a5f]/30" : "bg-[#1e3a5f]/[0.07] text-[#1e3a5f] border-[#1e3a5f]/20"
                    }`}>{g}</span>
                  ))}
                  {prefTypes.map((t) => (
                    <span key={t} className={`px-2.5 py-1 rounded-lg text-[11px] font-semibold border capitalize ${
                      dark ? "bg-white/[0.05] text-white/40 border-white/[0.08]" : "bg-gray-100 text-gray-500 border-gray-200"
                    }`}>{t.replace(/_/g, " ")}</span>
                  ))}
                  <button
                    onClick={() => setShowPrefModal(true)}
                    className={`ml-auto text-[11px] font-semibold shrink-0 hover:underline ${
        ...
        <section className="pb-4">
          <div className="flex items-center justify-between gap-4 mb-5">
            <h2 className={`text-xl font-semibold tracking-tight ${dark ? "text-gray-100" : "text-gray-800"}`}>
              Latest Scripts
            </h2>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mb-6 scrollbar-hide">
            ...
            {categories.contentTypes.map((c) => (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all capitalize border ${
          ...
          {loading ? (
            ...
            <motion.div
              ...
              className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-5"
            >
              {displayScripts.map((s, i) => {
                ...
                return (
                  <div key={s._id} className="relative">
                    {isMatch && (
                      <div className={`absolute -top-1.5 left-2 z-10 flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider shadow-sm ${
            ...
            <div className="text-center py-16">
              ...
              <p className={`font-normal text-sm ${dark ? "text-gray-500" : "text-gray-400"}`}>No scripts found</p>
            </div>
          )}
        </section>
