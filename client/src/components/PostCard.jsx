    ...
    >
      ...
      <div className="flex items-center justify-between p-4">
        <Link to={`/profile/${post.user._id}`} className="flex items-center gap-3 hover:opacity-80 transition">
          ...
          />
          <div>
            <h3 className="font-bold text-gray-900 text-base">{post.user.name}</h3>
            <p className="text-sm text-gray-500 capitalize font-medium">{post.user.role}</p>
          </div>
        </Link>
      ...
      <div className="px-4 py-3 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="text-gray-700 hover:text-[#1e3a5f] transition"
          >
            {saved ? (
