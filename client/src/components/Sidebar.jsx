...
const Sidebar = ({ purchaseRequestCount = 0, unreadMessageCount = 0 }) => {
  ...
  const fetchMyScripts = async () => {
    try {
      ...
      const mine = data.filter(
        (s) =>
          (s.creator?._id === user?._id || s.creator === user?._id) &&
          s.status !== "draft"
      );
  ...
  const mainNavItems = isAdmin
    ? [
        { path: "/search", label: "Search Projects" },
        { path: "/top-list", label: "Top List" },
        { path: "/featured", label: "Featured" },
      ]
    : isReader
    ? [
        { path: "/reader", label: "Home" },
        { path: `/profile/${user?._id || ""}`, label: "Profile" },
        { path: "/search", label: "Search Projects" },
        { path: "/top-list", label: "Top List" },
      ]
    : isIndustry
    ? [
        ...
        { path: `/profile/${user?._id || ""}`, label: "Profile" },
        { path: "/search", label: "Search Scripts" },
        { path: "/messages", label: "Messages" },
      ]
    : [
        { path: "/dashboard", label: "Dashboard" },
        ...(isWriterRole ? [{ path: "/featured", label: "Featured" }] : []),
        { path: "/top-list", label: "Top List" },
        { path: "/credits", label: "Credits" },
      ];
      ...
      <nav className="flex-1 overflow-y-auto">
        ...
        {mainNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`block px-6 py-3 text-sm font-semibold transition ${
                active
                  ? "text-blue-600"
                  : isDarkMode
        ...
        {!isIndustry && !isReader && (
          <>
            <button
              onClick={() => setProjectsOpen(!projectsOpen)}
              className="w-full text-left px-6 py-3 text-xs font-bold uppercase tracking-wide text-gray-400"
            >
              My Projects
            </button>
                ...
                {myScripts.length > 0 ? (
                  myScripts.map((script) => (
                    <Link
                      ...
                      className="block px-6 py-2 text-sm text-gray-500 hover:text-gray-900"
                    >
                      {script.title}
                    </Link>
                  ))
                ) : (
                  <p className="px-6 py-2 text-sm italic text-gray-400">
                    No projects yet
                  </p>
                )}
              </div>
