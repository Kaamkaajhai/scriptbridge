...
const MainLayout = ({ children }) => {
  ...
  const getNotifColor = (type) => {
    const colors = {
      ...
      follow: "text-[#1e3a5f] bg-[#1e3a5f]/10",
      unlock: "text-green-600 bg-green-50",
      hold: "text-amber-600 bg-amber-50",
      hold_expiring: "text-orange-500 bg-orange-50",
      script_score: "text-indigo-500 bg-indigo-50",
      trailer_ready: "text-purple-500 bg-purple-50",
      audition: "text-teal-500 bg-teal-50",
      smart_match: "text-violet-500 bg-violet-50",
      profile_view: "text-gray-500 bg-gray-100",
    };
                ...
                <div className="flex-1 overflow-y-auto">
                  {notifLoading ? (
                    ...
                    <div>
                      {notifications.map((n) => (
                        <div key={n._id}
                          className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 hover:bg-gray-50/50 transition-colors group ${
                            !n.read ? "bg-[#1e3a5f]/[0.02]" : ""
                          }`}>
                          {/* Icon */}
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-1 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
              ...
              <button onClick={() => { logout(); navigate("/login"); }}
                className="w-full text-left px-3 py-2.5 text-sm font-medium text-red-500 hover:bg-red-50 flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
